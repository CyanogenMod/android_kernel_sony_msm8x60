				...
				struct pm8921_rbatt_params *raw)
{
	...
	chip->read_vbatt_for_rbatt_param = raw->vbatt_for_rbatt_uv;
	chip->read_ocv_for_rbatt_param = raw->ocv_for_rbatt_uv;
	chip->read_vsense_for_rbatt_param = raw->vsense_for_rbatt_uv;
	return 0;
}
...
static void readjust_rbatt_table(int rbatt, int rbatt_from_lut)
{
	the_chip->ratio_to_default_rbatt =
		div_u64(rbatt * DEFAULT_RATIO, rbatt_from_lut);
...
static int get_rbatt(struct pm8921_bms_chip *chip, int soc_rbatt, int batt_temp)
{
	...
	if (chip->rbatt_temp_soc_lut) {
		...
		if (rbatt < 0) {
			...
			rbatt *= chip->ratio_to_default_rbatt;
			rbatt /= DEFAULT_RATIO;
					...
					int batt_temp, int chargecycles)
{
	...
	if (last_soc == -EINVAL || soc <= last_soc) {
		 ...
		 * If soc is allowed to increase during charging then only
		 * increase if one is 2 units greater than previous to supress
		 * jitter.
		 */
	...
	*val = 0;
	...
	case GET_VBAT_VSENSE_SIMULTANEOUS:
		...
		*val =
		...
		break;
	case REMEMBER_SOC:
		*val = the_chip->enable_remember_soc;
		break;
	default:
		ret = -EINVAL;
	}
...
static int set_calc(void *data, u64 val)
{
	...
	case START_OCV:
		...
		break;
	case REMEMBER_SOC:
		the_chip->enable_remember_soc = val;
		break;
	default:
		ret = -EINVAL;
	}
...
static void create_debugfs_entries(struct pm8921_bms_chip *chip)
{
	...
	debugfs_create_file("remember_soc", 0644, chip->dent,
				(void *)REMEMBER_SOC, &calc_fops);
...
static int __devinit pm8921_bms_probe(struct platform_device *pdev)
{
	...
	chip->enable_remember_soc = pdata->enable_remember_soc;
