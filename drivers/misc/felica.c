...
static int snfc_ldo_open(struct inode *inode, struct file *file)
{
	struct miscdevice *c = file->private_data;
	struct felica_dev *d = container_of(c, struct felica_dev, device_ldo);

	dev_dbg(d->device_ldo.this_device, "%s\n", __func__);

	return 0;
}

static int snfc_ldo_release(struct inode *inode, struct file *file)
{
	struct miscdevice *c = file->private_data;
	struct felica_dev *d = container_of(c, struct felica_dev, device_ldo);
...
static ssize_t snfc_ldo_write(struct file *file, const char __user *buf,
					size_t count, loff_t *offset)
{
	int ret;
	char kbuf;
	...
	if (1 != count || buf == NULL) {
		dev_err(d->device_ldo.this_device,
				"%s: Error. Invalid arg @LDO write.\n",
					__func__);
		ret = -EINVAL;
		goto exit;
	}
	...
	if (1 == kbuf) {
		d->flldo->ldo_write(1, d);
	} else if (0 == kbuf) {
		d->flldo->ldo_write(0, d);
	} else {
		dev_err(d->device_ldo.this_device,
			"%s: Error. Invalid val @ldo write.\n", __func__);
		ret = -EINVAL;
		goto exit;
	}
...
static const struct file_operations snfc_ldo_fops = {
	.owner		= THIS_MODULE,
	.read		= NULL,
	.write		= snfc_ldo_write,
	.open		= snfc_ldo_open,
	.release	= snfc_ldo_release,
	.fsync		= NULL,
};

static int snfc_ldo_probe_func(struct felica_dev *dev)
{
	int ret;

	dev_dbg(dev->dev, "%s\n", __func__);

	ret = dev->flldo->ldo_init(dev);
	if (ret) {
		dev_err(dev->dev, "%s: Error. ldo_init failed.\n",
				__func__);
		return -ENODEV;
	}
	if (misc_register(&dev->device_ldo)) {
		dev_err(dev->dev, "%s: Error. Cannot register SNFC ldo.\n",
				__func__);
		return -ENODEV;
	}
...
static void snfc_ldo_remove_func(struct felica_dev *dev)
{
	dev_dbg(dev->dev, "%s\n", __func__);

	misc_deregister(&dev->device_ldo);
	dev->flldo->ldo_release(dev);
}
...
static int felica_probe(struct platform_device *pdev)
{
	...
	dev->device_ldo.minor = MISC_DYNAMIC_MINOR;
	dev->device_ldo.name = "snfc_ldo";
	dev->device_ldo.fops = &snfc_ldo_fops;
	...
	ret = snfc_ldo_probe_func(dev);
	if (ret) {
		dev_err(&pdev->dev, "%s: SNFC ldo probe failure\n",
				__func__);
		goto err_snfc_ldo_probe;
	}
