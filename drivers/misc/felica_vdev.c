...
static long felica_vsub_cmd_ioctl(enum felica_cmd_type cmd_type, unsigned long arg)
{
	int ret;
	int error = 0;
...
static long felica_vsub_ioctl(struct file *file, enum felica_cmd cmd,
			unsigned long arg)
{
	int error = 0;
...
static int felica_vdev_fsync(struct file *file, int datasync)
{
	pr_debug("%s:[s]\n", __func__);
	if (!vdev_data->vsub_ref)
		return -EBADF;
